...
y = np.array(Labels)
print(x.shape)
np.save("./data/CV05_2_3_x_2",arr=x)
np.save("./data/CV05_2_3_y_2",arr=y)

# x = np.load("./data/CV05_2_3_x.npy")
# y = np.load("./data/CV05_2_3_y.npy")
...
lr = ReduceLROnPlateau(monitor='val_loss',
                                ...
                                verbose=1)
# ModelCheckPoint
modelpath = "./model/CV05_2_3_2_MCP_1214-{epoch:02d}-{val_loss:.4f}.hdf5" 
mcp = ModelCheckpoint(filepath=modelpath, monitor='val_loss',
                      save_best_only=True, mode="auto")

model.compile(loss="categorical_crossentropy",optimizer="adam",metrics="acc")
model.fit(x_train,y_train,epochs=10000,batch_size=64,validation_split=0.5,callbacks=[es,mcp,lr],verbose=1)
